...
def _build_detail_message(
    ...
    lines = []
    lines.append(f":bar_chart: *SEO 주간 모니터링 | {today_str}*")
    lines.append("")
    lines.append(f":speech_balloon: *Claude 진단*")
    lines.append(analysis.get("summary", "요약 없음"))
    lines.append("")
    lines.append("━━━━━━━━━━━━━━━━")
    lines.append(":chart_with_upwards_trend: *주요 지표*")
    lines.append("━━━━━━━━━━━━━━━━")
    lines.append(f":card_index_dividers: 색인: {indexed}개{indexed_diff}")
    lines.append(f":warning: 색인 오류: {index_errors}건{errors_diff}")
    lines.append("")
    lines.append(":zap: *Core Web Vitals*")
    ...
    lines.append("")
    lines.append("━━━━━━━━━━━━━━━━")
    lines.append(f":red_circle: *신규 이슈 ({len(new_issues)}건)*")
    if new_issues:
        for issue in new_issues:
            ...
            lines.append(f"• [{severity}] {url} - {desc}")
            if action:
    ...
    lines.append("")
    lines.append(f":large_yellow_circle: *재발 이슈 ({len(recurred)}건)*")
    if recurred:
    ...
    lines.append("")
    lines.append(f":white_check_mark: *해소된 이슈 ({len(resolved)}건)*")
    if resolved:
